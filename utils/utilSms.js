...
// 인증을 위해 발급받은 본인의 API Key를 사용합니다.
    ...
    const params = {
        text: `[테스트 메세지] 두근두근 라치오스 매칭에 성공하였습니다! 상대팀 대표의 카카오톡 아이디는 ${content}입니다`,
...
// 인증을 위해 발급받은 본인의 API Key를 사용합니다.
    ...
    const params = {
        text: `[테스트 메세지] 아쉽게도 성비 불균형으로 인해 두근두근 라치오스 매칭에 실패했습니다. 이번 이벤트에 관심 가져주셔서 감사합니다.`,
        type: 'SMS', // 발송할 메시지 타입 (SMS, LMS, MMS, ATA, CTA)
        to: num, // 수신번호 (받는이)
        from: '01091853702' // 발신번호 (보내는이)
    };
...
module.exports = {
    ...
    smsSend: async team => {
        try {
            await matchSuccessSms(team.leader.phoneNumber, team.partnerTeam.leader.kakaoId);
